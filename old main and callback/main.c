...
// Expose internal TinyUSB device-side endpoint handlers to main.c
extern bool usbd_edpt_busy(uint8_t rhport, uint8_t ep_addr);
extern bool usbd_edpt_claim(uint8_t rhport, uint8_t ep_addr);
extern bool usbd_edpt_xfer(uint8_t rhport, uint8_t ep_addr, uint8_t* buffer, uint16_t total_bytes, bool is_isr);
...
// Global downstream transport staging buffer array
static uint8_t downstream_tx_frame[26] = {0};
static const uint8_t sbc_in_endpoint = 0x81; // Endpoint 1 (IN)

// Asynchronous execution driver callback hook
bool sbch_device_in_xfer_cb(uint8_t rhport, uint8_t ep_addr, xfer_result_t result, uint32_t xferred_bytes)
{
    (void)rhport;
    (void)xferred_bytes;

    // Only queue a replacement frame if the prior transaction finished successfully
    if (ep_addr == sbc_in_endpoint && result == XFER_RESULT_SUCCESS)
    {
        Gamepad *gp = gamepad(0);
        
        // Transform current local structure layouts straight into the 26-byte XID container
        pack_steel_battalion_report(downstream_tx_frame, gp);
        
        // Safely stage data frames down the raw pipeline without saturating CPU loops
        usbd_edpt_claim(0, sbc_in_endpoint);
        usbd_edpt_xfer(0, sbc_in_endpoint, downstream_tx_frame, 26, false);
    }
    return true;
}
...
/*         tusb_gamepad_task();
        ...
        // =========================================================================
        // DIRECT XID SYNC FRAME TRANSMISSION PIPELINE
        // =========================================================================
        // Bypasses desktop HID framing entirely to interface directly with console hardware
        uint8_t const sbc_in_endpoint = 0x81; // Endpoint 1 (IN)
        
        if (tud_mounted() && !usbd_edpt_busy(0, sbc_in_endpoint)) 
        {
            uint8_t downstream_tx_frame[26];
            
            // Build the un-encapsulated pure 26-byte XID transaction block
            pack_steel_battalion_report(downstream_tx_frame, gp);
            
            // Claim and safely write directly to the raw endpoint buffer pipeline
            usbd_edpt_claim(0, sbc_in_endpoint);
            usbd_edpt_xfer(0, sbc_in_endpoint, downstream_tx_frame, 26, false);
        }
...
// Locate your application driver setup function and rewrite it exactly like this:
usbh_class_driver_t const* usbh_app_driver_get_cb(uint8_t* driver_count)
{
    static usbh_class_driver_t const custom_driver = {
        ...
        .xfer_cb    = sbch_device_in_xfer_cb, // Hook device pipeline events here!
        .close      = sbch_close
    };
...
void pack_steel_battalion_report(uint8_t *out_buf, Gamepad *gp) 
{
    ...
    // Original Xbox XID Standard structural metadata headers
    out_buf[0] = 0x00; 
    out_buf[1] = 0x1A; // Standard Steel Battalion payload expects exact boundary size tracking value (26 bytes total)

    // Reconstruct the 40-bit (5 byte) digital button array completely clean
    uint64_t b = 0;
    ...
    // Map 8-bit pieces incrementally into chronological order
    out_buf[2] = (uint8_t)(b & 0xFF);
    out_buf[3] = (uint8_t)((b >> 8) & 0xFF);
    out_buf[4] = (uint8_t)((b >> 16) & 0xFF);
    out_buf[5] = (uint8_t)((b >> 24) & 0xFF);
    out_buf[6] = (uint8_t)((b >> 32) & 0xFF); // Clean, uncorrupted byte sequence shift mapping
    out_buf[7] = 0x00;                        

    // Map analog axis positions matching the hardware definitions
    out_buf[8]  = 0x00; // Alignment spacing
    ...
    // Linear Foot Pedal mapping data bounds
    out_buf[18] = 0x00;
